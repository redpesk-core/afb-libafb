...
static void dispose_dataids_v2(void *closure, void *arg)
{
	afb_data_array_unref((unsigned)(intptr_t)closure, (struct afb_data**)arg);
}

static int send_resource_create_v2(struct afb_stub_rpc *stub, uint16_t id, const char *value, uint16_t kind)
{
	afb_rpc_v2_msg_resource_create_t creres;

	creres.kind = kind;
	creres.id = id;
	creres.data = (void*)value;
	creres.length = value ? (uint16_t)(1 + strlen(value)) : 0; /* TODO check size */
	return afb_rpc_v2_code_resource_create(&stub->coder, &creres);
}

static int send_resource_destroy_v2(struct afb_stub_rpc *stub, uint16_t id, uint16_t kind)
{
	afb_rpc_v2_msg_resource_destroy_t desres;

	desres.kind = kind;
	desres.id = id;
	return afb_rpc_v2_code_resource_destroy(&stub->coder, &desres);
}

static int send_session_create_v2(struct afb_stub_rpc *stub, uint16_t id, const char *value)
{
	return send_resource_create_v2(stub, id, value, AFB_RPC_V2_ID_KIND_SESSION);
}

static int send_token_create_v2(struct afb_stub_rpc *stub, uint16_t id, const char *value)
{
	return send_resource_create_v2(stub, id, value, AFB_RPC_V2_ID_KIND_TOKEN);
}

static int send_event_create_v2(struct afb_stub_rpc *stub, uint16_t id, const char *value)
{
	return send_resource_create_v2(stub, id, value, AFB_RPC_V2_ID_KIND_EVENT);
}

static int send_event_destroy_v2(struct afb_stub_rpc *stub, uint16_t id)
{
	return send_resource_destroy_v2(stub, id, AFB_RPC_V2_ID_KIND_EVENT);
}

static int send_event_unexpected_v2(struct afb_stub_rpc *stub, uint16_t id)
{
	afb_rpc_v2_msg_event_unexpected_t msg = { .eventid = id };
	return afb_rpc_v2_code_event_unexpected(&stub->coder, &msg);
}

static int send_event_push_v2(struct afb_stub_rpc *stub, uint16_t eventid, unsigned nparams, struct afb_data * const params[])
{
	afb_rpc_v2_msg_event_push_t push;
	...
	if (rc >= 0) {
		...
		if (rc >= 0) {
			...
			afb_rpc_coder_on_dispose2_output(&stub->coder, dispose_dataids_v2, (void*)(intptr_t)nparams, (void*)params);
		}
	}
	return rc;
}

static int send_event_broadcast_v2(struct afb_stub_rpc *stub, const char *eventname, unsigned nparams, struct afb_data * const params[], const unsigned char uuid[16], uint8_t hop)
{
/* TODO */return 0;
}

static int send_event_subscribe_v2(struct afb_stub_rpc *stub, uint16_t callid, uint16_t eventid)
{
	afb_rpc_v2_msg_event_subscribe_t sub;
	sub.callid = callid;
	sub.eventid = eventid;
	return afb_rpc_v2_code_event_subscribe(&stub->coder, &sub);
}

static int send_event_unsubscribe_v2(struct afb_stub_rpc *stub, uint16_t callid, uint16_t eventid)
{
	afb_rpc_v2_msg_event_unsubscribe_t sub;
	sub.callid = callid;
	sub.eventid = eventid;
	return afb_rpc_v2_code_event_unsubscribe(&stub->coder, &sub);
}

static int send_call_reply_v2(struct afb_stub_rpc *stub, int status, unsigned nparams, struct afb_data * const params[], uint16_t callid)
{
	afb_rpc_v2_msg_call_reply_t reply;
	afb_rpc_v2_value_t values[nparams];
	afb_rpc_v2_value_array_t valarr = { .count = (uint16_t)nparams, .values = values };;
	...
	if (rc >= 0) {
		...
		rc = afb_rpc_v2_code_call_reply(&stub->coder, &reply, &valarr);
		if (rc >= 0) {
			afb_data_array_addref(nparams, params);
			afb_rpc_coder_on_dispose2_output(&stub->coder, dispose_dataids_v2, (void*)(intptr_t)nparams, (void*)params);
		}
...
static int send_call_request_v2(
	struct afb_stub_rpc *stub,
	uint16_t callid,
	uint16_t sessionid,
	uint16_t tokenid,
	...
	struct afb_data * const params[])
{
	afb_rpc_v2_msg_call_request_t request;
	afb_rpc_v2_value_t values[nparams];
	afb_rpc_v2_value_array_t valarr = { .count = (uint16_t)nparams, .values = values };;
	...
	if (rc >= 0) {
		...
		rc = afb_rpc_v2_code_call_request(&stub->coder, &request, &valarr);
		if (rc >= 0) {
			afb_data_array_addref(nparams, params);
			afb_rpc_coder_on_dispose2_output(&stub->coder, dispose_dataids_v2, (void*)(intptr_t)nparams, (void*)params);
		}
...
static int send_describe_request_v2(struct afb_stub_rpc *stub, uint16_t callid)
{
	afb_rpc_v2_msg_call_request_t request;

	memset(&request, 0, sizeof request);
	request.callid = callid;
	request.verb.id = AFB_RPC_V2_ID_VERB_DESCRIBE;
	return afb_rpc_v2_code_call_request(&stub->coder, &request, NULL);
}

static int send_describe_reply_v2(struct afb_stub_rpc *stub, uint16_t callid, const char *description)
{
	return afb_rpc_v1_code_description(&stub->coder, callid, description);
	afb_rpc_v2_value_t value;
	afb_rpc_v2_value_array_t valarr = { .count = 1, .values = &value };;
	afb_rpc_v2_msg_call_reply_t reply;
...
static int typed_value_to_data_v2(struct afb_stub_rpc *stub, uint16_t typenum, uint32_t length, const void *value, struct afb_data **data)
{
	...
	case AFB_RPC_V2_ID_TYPE_OPAQUE:
		...
		break;
	case AFB_RPC_V2_ID_TYPE_BYTEARRAY:
		type1 = &afb_type_predefined_bytearray;
		break;
	case AFB_RPC_V2_ID_TYPE_STRINGZ:
		type1 = &afb_type_predefined_stringz;
		break;
	case AFB_RPC_V2_ID_TYPE_JSON:
		type1 = &afb_type_predefined_json;
		break;
	case AFB_RPC_V2_ID_TYPE_BOOL:
		size = 1;
		type2 = &afb_type_predefined_bool;
		break;
	case AFB_RPC_V2_ID_TYPE_I32:
		size = 4;
		type2 = &afb_type_predefined_i32;
		break;
	case AFB_RPC_V2_ID_TYPE_U32:
		size = 4;
		type2 = &afb_type_predefined_u32;
		break;
	case AFB_RPC_V2_ID_TYPE_I64:
		size = 8;
		type2 = &afb_type_predefined_i64;
		break;
	case AFB_RPC_V2_ID_TYPE_U64:
		size = 8;
		type2 = &afb_type_predefined_u64;
		break;
	case AFB_RPC_V2_ID_TYPE_DOUBLE:
		size = 8;
...
static int decode_call_request_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_call_request_t *msg, afb_rpc_v2_value_array_t *values)
{
	int rc;
	const char *verb;
	...
	if (verb == NULL) {
		...
		switch (msg->verb.id) {
		case AFB_RPC_V2_ID_VERB_DESCRIBE:
...
static int decode_call_reply_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_call_reply_t *msg, afb_rpc_v2_value_array_t *values)
{
	int rc;
	unsigned count = values->count;
	struct afb_data *datas[count];
...
static int decode_event_push_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_event_push_t *msg, afb_rpc_v2_value_array_t *values)
{
	int rc;
	unsigned count = values->count;
	struct afb_data *datas[count];
...
static int decode_event_subscribe_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_event_subscribe_t *msg)
{
	return receive_event_subscribe(stub, msg->callid, msg->eventid);
}

static int decode_event_unsubscribe_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_event_unsubscribe_t *msg)
{
	return receive_event_unsubscribe(stub, msg->callid, msg->eventid);
}

static int decode_event_unexpected_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_event_unexpected_t *msg)
{
	return receive_event_unexpected(stub, msg->eventid);
}

static int decode_event_broadcast_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_event_broadcast_t *msg, afb_rpc_v2_value_array_t *values)
{
	int rc;
	unsigned count = values->count;
	struct afb_data *datas[count];
...
static int decode_resource_create_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_resource_create_t *msg)
{
	int rc = 0;
	switch (msg->kind) {
	...
	case AFB_RPC_V2_ID_KIND_TOKEN:
		...
		break;
	case AFB_RPC_V2_ID_KIND_EVENT:
		rc = receive_event_create(stub, msg->id, msg->data);
		break;
	case AFB_RPC_V2_ID_KIND_API:
	case AFB_RPC_V2_ID_KIND_VERB:
	case AFB_RPC_V2_ID_KIND_TYPE:
	case AFB_RPC_V2_ID_KIND_DATA:
	case AFB_RPC_V2_ID_KIND_KIND:
	case AFB_RPC_V2_ID_KIND_CREDS:
	case AFB_RPC_V2_ID_KIND_OPERATOR:
		rc = X_ENOTSUP;
		break;
	}
	return rc;
...
static int decode_resource_destroy_v2(struct afb_stub_rpc *stub, afb_rpc_v2_msg_resource_destroy_t *msg)
{
	int rc = 0;
	switch (msg->kind) {
	case AFB_RPC_V2_ID_KIND_SESSION:
...
static int decode_v2(struct afb_stub_rpc *stub)
{
	afb_rpc_v2_pckt_t packet;
	afb_rpc_v2_msg_t msg;
	afb_rpc_v2_value_t values[64];
	afb_rpc_v2_value_array_t valarr;

	int rc = afb_rpc_v2_decode_packet(&stub->decoder, &packet);
	if (rc >= 0) {
		if (afb_rpc_v2_is_single_packet_std_oper(&packet)) {
			valarr.values = values;
			valarr.count = sizeof values / sizeof *values;
			msg.values.array = &valarr;
			msg.values.allocator = NULL;
			rc = afb_rpc_v2_decode_single_packet_std_oper(&packet, &msg);
			if (rc >= 0) {
				switch(msg.oper) {
				case AFB_RPC_V2_ID_OP_CALL_REQUEST:
					rc = decode_call_request_v2(stub, &msg.head.call_request, &valarr);
					break;
				case AFB_RPC_V2_ID_OP_CALL_REPLY:
					rc = decode_call_reply_v2(stub, &msg.head.call_reply, &valarr);
					break;
				case AFB_RPC_V2_ID_OP_EVENT_PUSH:
					rc = decode_event_push_v2(stub, &msg.head.event_push, &valarr);
					break;
				case AFB_RPC_V2_ID_OP_EVENT_SUBSCRIBE:
					rc = decode_event_subscribe_v2(stub, &msg.head.event_subscribe);
					break;
				case AFB_RPC_V2_ID_OP_EVENT_UNSUBSCRIBE:
					rc = decode_event_unsubscribe_v2(stub, &msg.head.event_unsubscribe);
					break;
				case AFB_RPC_V2_ID_OP_EVENT_UNEXPECTED:
					rc = decode_event_unexpected_v2(stub, &msg.head.event_unexpected);
					break;
				case AFB_RPC_V2_ID_OP_EVENT_BROADCAST:
					rc = decode_event_broadcast_v2(stub, &msg.head.event_broadcast, &valarr);
					break;
				case AFB_RPC_V2_ID_OP_RESOURCE_CREATE:
					rc = decode_resource_create_v2(stub, &msg.head.resource_create);
					break;
				case AFB_RPC_V2_ID_OP_RESOURCE_DESTROY:
					rc = decode_resource_destroy_v2(stub, &msg.head.resource_destroy);
					break;
				default:
					rc = X_EPROTO;
					break;
